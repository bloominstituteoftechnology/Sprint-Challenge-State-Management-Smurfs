...
- [x] What problem does the context API help solve?
      ...
      Context API works to keeo your state clean by sharing values between components without explicitly passing a prop through every level of the tree. Data can jump around a little more easily.

- [x] In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?

      An action is dispatched with a payload and runs through a reducer, which takes the ( current state + action ) to return a new state. THe store is where all of that state is stored, including global data to be accessed across different compoenents. 

- [x] What is the difference between Application state and Component state? When would be a good time to use one over the other?

      An application state can be accessed globally, whereas a component state has a smaller scope, reaching only within its component. Keep as little code as possible from being repeated (DRY) by putting more reusable code in the application state. 

- [x] Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?

      Redux-thunk allows us to use action creators as functions instead of just an object, it serves as a "middle-man" by being able to read everything and passing it through, whereas without it anything that's not a pure function or has side-effects will not work. 

- [x] What is your favorite state management system you've learned and this sprint? Please explain why!

      Context is easier but I did not spend enough time with it so I am more comfortable with redux. 
