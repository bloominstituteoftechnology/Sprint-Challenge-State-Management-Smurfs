...
A.Context API helps eliminate the need to use props for global data that can be used through multiple components.


- [x] In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?

A.In redux:
 -Actions are used to store data types as well as the information that goes along with that type.
-Reducers are pure functions that are used to manage state changes in a predictable manner.
-The redux store is used to encapsulize state used in apps so that all changes in state only need one reference to go by (which is the store).  This reduces the need to create many useState hooks and rely on only one source.  Which is why the store is also known as a 'single source of truth'.
...
A. I enjoyed context API the best because it's simple and helps eliminate the need for passing a bunch of props which can get redundant at times.
