...
- [x] In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?
      a.	Actions – information hub that sends from the app to the store
      b.	Reducers – explains the states changes in response to the actions
      c.	Store – brings the reducers & actions together
      d.	Because that’s where all the info needed for the app is stored

- [x] What is the difference between Application state and Component state? When would be a good time to use one over the other?
      a.	Application State is global – with store any component within the app can access the application state
      b.	Component State is local – state lives in that specific component and can only be updated via that component & passed down to children via props
      c.	When state doesn’t need to be shared with other components use Component State. When you are fetching from API’s or data files, you want to use Application State

- [x] Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?
      a.	Allows us to handle asynchronous operations within our actions. Allows us to call actions that reurns functions instead of action objects via dispatch.
- [x] What is your favorite state management system you've learned and this sprint? Please explain why!
      a.	Context api! Its just easier for me to grasp
