...
from having to drill down in multiple components, in larger apps helps increase effeciency,
though can make things harder for the programmer
- [ ] In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?
Reducers take things that manipulate the store "state" and puts them in a single location, that can be triggered by dispatches found within "Actions". Store is a Single set of state for multiple components
- [ ] What is the difference between Application state and Component state? When would be a good time to use one over the other?
Application state is the overall state, that includes component state.
Component state is that slice of state only "seen" and handled by a component.
If a component does not need to pull in the whole application state, then you shouldn't for efficiency-sake.
- [ ] Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?
instead of passing objects, we can pass through functions, allows us to put logic within the action creators
- [ ] What is your favorite state management system you've learned and this sprint? Please explain why!
Context API, was easier to grasp, and I like the ability to choose what the component state sees
