...
> Prop-drilling. The act of passing props from parent to child on multiple levels.
- [x] In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?
> the store is where the actual state lives. reducers are how the state changes. actions are how the app triggers one of those changes. the store is referred as 'single source of truth' as the store consolidates your application's state.
- [x] What is the difference between Application state and Component state? When would be a good time to use one over the other?
> Application state is some piece of state that the whole app (or most of it) needs. Component state is largely something that is specific to a component, e.g. the current state of a particular form, where that state only makes sense for that component.
- [x] Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?
>`redux-thunk` allows us to inject some code based on the context of another action. The `action-creators` can now return a function (a thunk), hence the name `redux-thunk`
- [x] What is your favorite state management system you've learned and this sprint? Please explain why!
> I like the combination of useState and Context API. It accomplishes most of what redux does and the boilerplate is minimal.
